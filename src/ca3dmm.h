...
struct ca3dmm_engine
{
    ...
    int  *C_out_rs_cnt;             // Size task_k_num, output C matrix block reduce-scatter receive count array
    ...
    MPI_Comm comm_glb;              // Communicator for all MPI processes
    MPI_Comm comm_mn_bcast;         // Communicator for m or n dimension broadcast
    MPI_Comm comm_k_reduce;         // Communicator for k dimension reduction
    ...
    double dupAB_ms;                // Time (milliseconds) used in duplication of A or B
